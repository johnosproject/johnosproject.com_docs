...
<WhatIntro
logo_path="/docs/img/icons/Maker_Icon_250.png">

A JOSP Object, at least, should be configured by **someone that define object's info and structure**.
...
* as physical objects:<br/>
  ...
  When plugged in, those objects startup the JOD agent to expose their feature to the JOSP EcoSystem
...
All JOD Distribution are configured to use specific firmware's tools (the object's structure define also the list of the firmware calls).
Because of that, **each JOD Distribution come with his specific requirements**.<br/>
...
For example a distribution can require a specific gateway reachable on the local network.
...
Then he prepares a JOD Distribution to install and run on the physical object.<br/>
...
Often, for JOSP Objects provided as physical objects, the **object's maker correspond with the product's manufacturer**.
