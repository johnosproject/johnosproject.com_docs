...
**To connect  a [JOSP Object](/docs/what/josp/object) to a [JOSP EcoSystem](/docs/what/josp/ecosystem), the JOD Agent use a complex communication system.**<br/>
This system is delegated to handle communication with JOSP Services (both [Direct](/docs/what/josp/specs/comm#direct) and [Cloud](/docs/what/josp/specs/comm#cloud)), manage JCP Authentication, but also to encrypt all tx/rx data.
...
[JCP APIs](/docs/references/josp/jcp/microservices/apis) are a set of HTTP Methods exposed by the JCP platform and are used by JOD Agent to handle the JOSP Object on the JCP.
...
To communicate with JOSP Services, the JOD Agent support both JOSP Communications types: [Direct](/docs/what/josp/specs/comm#direct) and [Cloud](/docs/what/josp/specs/comm#cloud).

For **Cloud Communication**, the JOD Agent open a connection to the [JCP Gateways](/docs/references/josp/jcp/microservices/gws).
...
The JOD Agent use internal [JOD Gateway O2S Client](#jod-gateway-o2s-client), that support [JOSP protocol](/docs/what/josp/specs/protocol), to connect with the JCP GW.
...
[Local JOD Server](#jod-local-server) support the [JOSP protocol](/docs/what/josp/specs/protocol) and is executed at JOD Agent startup.<br/>
...
Also the [Public JCP](/docs/references/josp/jcp/public_jcp), with the aim to improve your security, expose only HTTPs endpoints and encrypt all communication with his own SSL certificate.
