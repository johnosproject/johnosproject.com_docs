...
The **object's structure** contained in the ```configs/struct.jod``` file is a hierarchical list of the features that the JOD Agent must expose to the JOSP EcoSystem.<br/>
...
More details and examples on [Pillars](pillars/) and [Workers](workers/) chapters.
...
Object's info are related to JOSP Object implementation and are contained in the ```struct.jod``` file as part of the object's structure.

You can use this info to define your JOSP Object and help JOSP Service developers find and use it.

From JOSP Service developers prospective, object's info are intended as model info exposed by JOSP Objects; and can help them to identify connected objects.
...
Please customize the main ```contains``` section according to the JOSP Object's structure you would expose.<br/>
