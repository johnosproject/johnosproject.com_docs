...
The internal instance can be stopped and the re-started with the [```jod-start``` command](#jod-start).
...
At JOD Agent startup, the JCP connection is initialized only if the property ```jcp.connect``` from [JOD Agent configs](configs) is set to 'true'.
...
At JOD Agent startup, the local server is started only if the property ```jod.comm.local.enabled``` from [JOD Agent configs](configs) is set to 'true'.
...
At JOD Agent startup, the gateway client is initialized only if the property ```jod.comm.cloud.enabled``` from [JOD Agent configs](configs) is set to 'true'.
