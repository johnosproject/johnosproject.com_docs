...
title: Puller Http
---
# JOD Worker Puller Http

Each ```freq``` seconds, this puller performs the HTTP Request defined via [HTTP Request](#firmware-configs---http-request) configs , then update the [JOD State](../pillars#state).

Before execute the request, the ```requestUrl``` string is updated and all his placeholder are replaced with current [Pillar](workers_placeholders#pillar) and [State](workers_placeholders#state) properties.
Once executed the HTTP Request, his response body is [formatted](#firmware-configs---formatter) and [evaluated](#firmware-configs---evaluator).
Finally, the evaluated HTTP Response's body is passed as new Pillar's state, independently to the Pillar's type.
...
Once a url is required by any instance of Puller Http worker, his response is retrained in a cache memory for ```cache_timeout``` seconds.<br/>
For all time a url's response is cached, all other request (from any HTTP Puller instance) to the same url recevie cached response.<br/>
That prevent HTTP server overload, especially when multiple HTTP Puller workers get their value from the same web page or API method.
...
HTTP Requests are performed using the [DefaultHTTPClient](https://bitbucket.org/johnosproject_shared/com.robypomper.josp/src/2.2.1/src/jospCommons/java/com/robypomper/josp/clients/DefaultHTTPClient.java) from the [JOSP Commons](http://localhost:3000/docs/references/josp/internal/josp_commons/) library.
...
The ```requestUrl``` string is updated during worker initialization and then each time the puller pulls for pillar state.
...
Once an HTTP Request receive a response, the response's body is parsed and formatted depending on those Firmware Configs.

That means, the HTTP Responses bodies are parsed using the ```formatType``` format.
Then can be extracted part of the response, using the ```formatPathType``` and ```formatPath``` configs.
Finally, only the extracted string is used as Pillar's state.

To avoid any response alteration, you can use the ```TEXT``` value in the ```formatType``` config.
That will thread the HTTP Response Body as raw text and, all contents from the response body are used as Pillar's state.

Those configs are defined and used by [FormatterInternal](https://bitbucket.org/johnosproject_shared/com.robypomper.josp/src/2.2.1/src/jospJOD/java/com/robypomper/josp/jod/executor/impls/http/FormatterInternal.java) class.
...
* ```TEXT```: raw text response, this format do not alter the response body.
...
Those values are coming from the [FormatterInternal::FormatType](https://bitbucket.org/johnosproject_shared/com.robypomper.josp/src/2.2.1/src/jospJOD/java/com/robypomper/josp/jod/executor/impls/http/FormatterInternal.java#lines-247) enum.

This property is updated with [Pillar's Placeholder](workers_placeholders#pillar) on worker initialization.

### ```formatPath```

Path used to query the HTTP Response, this value depends on ```formatPathType``` Firmware Config. Default '/'.
...
Those values are coming from the [FormatterInternal](https://bitbucket.org/johnosproject_shared/com.robypomper.josp/src/2.2.1/src/jospJOD/java/com/robypomper/josp/jod/executor/impls/http/FormatterInternal.java#lines-69) class.
...
If you are not yet satisfy from the result after the HTTP Response body format, you can continue customizing it within a configurable JavaScript script.

For example, if after response format, it still contains unwanted chars; or also if you configured the worker for handle a raw response  (```formatType=TEXT```).
Then you need to edit the obtained response before passing it as new Pillar's state value.

To edit obtained result, after the response's body format, you must use the ```eval``` Firmware Config.<br/>
Default ```eval``` value, or an empty string, prevents any response alteration.

Those configs are defined and used by [EvaluatorInternal](https://bitbucket.org/johnosproject_shared/com.robypomper.josp/src/2.2.1/src/jospJOD/java/com/robypomper/josp/jod/executor/impls/http/EvaluatorInternal.java) class.

### ```eval```

JavaScript script to evaluate HTTP Response body after formatting it. Default '{httpResult}'.

This config accept any JavaScript script and response the evaluation function returns his output.

Custom JavaScript can use the ```httpResult``` string that contains the HTTP Response body after formatting.<br/>
